...
sap.ui.define(["sap/fe/core/AppComponent", "sap/m/MessageBox"], function (Component, MessageBox) {
  "use strict";
  ...
  return Component.extend("c4p.mdg.mdgcreaterequest.Component", {
    ...
    _getStartupParameter: function (sParameterName) {
      ...
      return new URLSearchParams(window.location.search).get(sParameterName) || new URLSearchParams(window.location.hash.split("?")[1] || "").get(sParameterName);
    },
    _handleStartupCreateRequest: function () {
      const sExternalSystem = this._getStartupParameter("ExternalSystem");
      const sPartnerGid = this._getStartupParameter("PartnerGID") || this._getStartupParameter("PartnerGid");
      ...
      this.getModel().getMetaModel().requestObject("/").then(function () {
        ...
        MessageBox.error(this._getText("createRequestDraftFailed"), {
    ...
    _createRequest: function (sExternalSystem, sPartnerGid) {
      return this._executeCreateRequest(sExternalSystem, sPartnerGid);
    },
    _executeCreateRequest: function (sExternalSystem, sPartnerGid) {
      const oModel = this.getModel();
      const oRequestsBinding = oModel.bindList("/Requests");
      const oOperation = oModel.bindContext("com.sap.gateway.srvd.zui_mdg_req.v0001.CreateRequest(...)", oRequestsBinding.getHeaderContext());
    ...
    _navigateToRequestByKey: function (sRequestUuid, bIsActiveEntity) {
      this._navigateToRequest("/Requests(RequestUuid=" + sRequestUuid + ",IsActiveEntity=" + (bIsActiveEntity === true ? "true" : "false") + ")");
    },
    _navigateToRequest: function (sCanonicalPath) {
      const sKeyPredicate = sCanonicalPath.replace(/^\/Requests\(/, "").replace(/\)$/, "");
